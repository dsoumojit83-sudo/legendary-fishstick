...
module.exports = async function (req, res) {
    ...
    // 🔒 SECURITY FIX: Also verify caller's JWT and match email to order owner.
    let callerEmail = null;
    const authHeader = req.headers['authorization'];
    if (authHeader?.startsWith('Bearer ')) {
        const { data: { user } } = await supabase.auth.getUser(authHeader.slice(7));
        if (user) callerEmail = (user.email || '').toLowerCase();
    }
    ...
    // Verify the caller owns this order (match email) or has a valid session
    if (callerEmail && existingOrder.client_email &&
        callerEmail !== existingOrder.client_email.toLowerCase()) {
        return res.status(403).json({ error: 'You are not authorized to upload to this order.' });
    }
