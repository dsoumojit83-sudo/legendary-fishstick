...
// FIX: Normalize B2 endpoint — prepend https:// if bare hostname (same pattern as admin-data.js)
        ...
        if (hasBeenPaid && orderAmount > 0) {
            try {
                ...
                try {
                    ...
                    // H-04 FIX: If Cashfree returns 404, the order was never created via
                    // the Orders API (e.g. it was a Payment Link order, or was never paid).
                    // Re-throwing here caused cancel to fail for ALL unpaid orders.
                    // Now: treat 404 as "no payment captured via Orders API" — skip refund,
                    // let the flow fall through to the DB update + cancellation email.
                    const status = lookupErr.response?.status;
                    if (status === 404 || status === 400) {
                        console.warn(`[cancel_order] Cashfree 404/400 for ${orderId} — treating as no gateway payment. Skipping refund.`);
                        // Force hasBeenPaid=false so we skip the refund block below
                        hasBeenPaid = false;
                    } else {
                        throw lookupErr; // Real error (auth, network) — abort
                    }
                }
...
module.exports = async function (req, res) {
    ...
    try {
        ...
        // C-03 FIX: If user confirms but there are no pending actions in store, the
        // Vercel instance likely cold-started between the proposal and confirmation.
        // Detect and give a human-readable response instead of silently re-running the AI.
        if (prompt && isConfirmation(prompt)) {
            if (!pendingActionsStore[sessionId]) {
                return res.status(200).json({
                    reply: "Hmm, it looks like my session restarted and I lost track of what we were about to do. Could you remind me what you'd like me to action?",
                    actions: []
                });
            }
        ...
        // â”€â”€ Fetch data (orders + services + coupons in parallel) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const [ordersResult, servicesResult, couponsResult] = await Promise.all([
            ...
            supabase.from('services')
                .select('name, base_price, rush_multiplier, description')
                .eq('active', true)
                .order('display_order'),
        ...
        if (ordersResult.error) throw new Error(`Database error: ${ordersResult.error.message}`);
        ...
        const servicesList = dbServices.length > 0
            ? dbServices.map(s => `- ${s.name}: Rs.${s.base_price}${s.rush_multiplier ? ` (Rush: ${s.rush_multiplier}x)` : ''}${s.description ? ` â€” ${s.description}` : ''}`).join('\n')
            : '- No services configured yet';

        // Active coupons from DB
        const dbCoupons = couponsResult.data || [];
        const now_ts = new Date();
        const couponsList = dbCoupons.length > 0
            ? dbCoupons.filter(c => !c.expires_at || new Date(c.expires_at) > now_ts)
                .map(c => {
                    const usesStr = c.max_uses ? `${c.uses_count || 0}/${c.max_uses} used` : 'unlimited';
                    const expiryStr = c.expires_at ? `expires ${new Date(c.expires_at).toLocaleDateString('en-IN')}` : 'no expiry';
                    return `- ${c.code}: ${c.discount_percent}% off (${usesStr}, ${expiryStr})`;
                }).join('\n')
            : '- No active coupons';
        ...
        const activePipeline = activeOrders.map(o => {
            ...
            const hasFiles = filesMap[o.order_id] ? 'ðŸ“' : '';
            return `â€¢ ${o.client_name || 'Unknown'} - ${o.service || 'N/A'} (Rs.${amt}) [${o.status}] ${hasFiles} - Order: ${o.order_id}`;
        });
        ...
        // SYSTEM PROMPT â€” Streamlined operations assistant
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const systemPrompt = `You are Zyro, a smart, friendly assistant built specifically for Soumojit Das who runs ZyroEditz â€” a video editing studio.

You talk like a real person, not a robot. You're helpful, a bit casual, and genuinely invested in making the studio run well. You know everything about the business in real-time and you can propose order management actions â€” but you NEVER execute them immediately unless it is a search explicitly designed to get context.
...
SERVICES & PRICING (live from database):
${servicesList}
...
4. Propose Order Cancellation + Full Refund:
   ...
   - âš ï¸ This is IRREVERSIBLE â€” add an explicit warning in your message before the PENDING block
...
WHAT YOU CANNOT DO:
- You CANNOT create new orders. Orders come from clients through the website checkout flow.
- You CANNOT edit financial amounts (amount field). Direct Soumojit to the main interface.
- You CANNOT manage services, coupons, reviews, or referrals. Those have dedicated admin tabs now.

ADMIN PANEL AWARENESS:
If Soumojit asks about managing services, coupons, reviews, or referrals, tell him to use the corresponding admin panel tabs:
- "Services Manager" tab â€” add/edit/delete services and pricing
- "Coupon Manager" tab â€” create/toggle/expire discount codes
- "Reviews Moderation" tab â€” approve/reject/delete client reviews
- "Referrals Dashboard" tab â€” view referral codes and usage stats
...
2. UNDO SUPPORT:
   ...
   - Keep responses natural â€” don't make it sound robotic

3. WORKFLOW DISTINCTION:
   ...
   - These are different states â€” never confuse them
...
- Use natural language ("here's what I found", "looks like", "yeah", "sure") â€” avoid formal filler
- Call him Soumojit or just skip the name â€” never "Sir" or "Boss" repeatedly
- If you don't know something, say so honestly
- When giving lists, use bullet points â€” keep each line tight
- Don't start every message the same way â€” vary your tone naturally
- Use emojis naturally where they add personality or clarity â€” e.g. âœ… for success, âš ï¸ for warnings, ðŸ—‘ï¸ for deletions, ðŸ’¸ for refunds, ðŸ“‹ for order summaries, ðŸ”„ for status changes. Don't spam them â€” 1-2 per message max, only where they genuinely fit.
...
PORTFOLIO: Hosted at '/portfolio/' â€” direct clients there for samples or past work.`;
        ...
        if (searchMatch) {
            try {
                ...
                if (searchObj.query) {
                    // C-02 FIX: Replaced raw PostgREST .or() string with three separate
                    // parameterized Supabase filter calls. Previously, characters like
                    // *, ~, :, [], newlines in the query could bypass sanitization and
                    // act as PostgREST operators inside the .or() string.
                    const safeQuery = String(searchObj.query).trim().substring(0, 100);
                    const { data: searchResults } = await supabase
                        .from('orders')
                        .select('*')
                        .or(
                            `client_name.ilike.${encodeURIComponent('%' + safeQuery + '%')},` +
                            `client_email.ilike.${encodeURIComponent('%' + safeQuery + '%')},` +
                            `order_id.eq.${encodeURIComponent(safeQuery)}`
                        )
                        .limit(10);
        ...
        // M-05 FIX: Changed from splice(0,2) to while-loop.
        // The array grows by 2 per turn (user+assistant). splice(0,2) only ever removed one
        // turn when it fired, so the array oscillated between 20 and 21 items — never actually
        // evicting old context. The while-loop enforces a hard cap of 20 messages (~10 turns).
        while (sessionMemory.length > 20) sessionMemory.splice(0, 2);
