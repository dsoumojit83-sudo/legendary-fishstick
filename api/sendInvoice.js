        ...
        try {
            ...
            doc.on("end", async () => {
                try {
                    ...
                    // Parse project_notes for multi-item display and coupon info
                    let cartItems = [];
                    let appliedCoupon = null;
                    try {
                        if (order.project_notes) {
                            const parsed = JSON.parse(order.project_notes);
                            if (Array.isArray(parsed)) {
                                cartItems = parsed; // Legacy format
                            } else if (parsed.items || parsed.coupon) {
                                cartItems = parsed.items || [];
                                appliedCoupon = parsed.coupon;
                            }
                        }
                    } catch (e) { /* fallback */ }
                    const isMultiItem = Array.isArray(cartItems) && cartItems.length > 1;
                    ...
                    if (isMultiItem) {
                        cartItems.forEach(item => {
                            emailItemRows += `
                                <tr>
                                    <td style="padding: 10px 0; color: #ffffff; font-size: 14px; border-top: 1px solid #222222;">${escapeHtml(item.service)}</td>
                                    <td style="padding: 10px 0; color: #ffffff; text-align: right; border-top: 1px solid #222222; font-size: 14px;">Rs.${parseFloat(item.price).toFixed(0)}</td>
                                </tr>`;
                        });
                    }
                    if (appliedCoupon) {
                         emailItemRows += `
                                <tr>
                                    <td style="padding: 10px 0; color: #22c55e; font-size: 14px; border-top: 1px solid #222222;">Discount (${appliedCoupon.code})</td>
                                    <td style="padding: 10px 0; color: #22c55e; text-align: right; border-top: 1px solid #222222; font-size: 14px;">-Rs.${parseFloat(appliedCoupon.discount_amount).toFixed(0)}</td>
                                </tr>`;
                    }
                                ...
                                <div style="padding: 40px 30px;">
                                    ...
                                    <div style="background-color: #111111; border: 1px solid #333333; border-radius: 8px; padding: 20px; margin: 30px 0;">
                                        <table style="width: 100%; border-collapse: collapse;">
                                            ...
                                            ${isMultiItem ? `
                                                <tr>
                                                    <td colspan="2" style="padding: 10px 0; color: #888888; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; border-top: 1px solid #222222;">Services Ordered</td>
                                                </tr>
                                                ${emailItemRows}
                                            ` : `
                                                <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td style="padding: 10px 0; color: #ffffff; font-weight: bold; text-align: right; border-top: 1px solid #222222; font-size: 14px;">Rs.${escapeHtml(String(order.amount))}</td>
                                            </tr>
                                            <tr>
            ...
            // ── TABLE ROW(S) — multi-item support ──
            let rowY = 323;
            if (isMultiItem) {
                cartItems.forEach((item, idx) => {
                    if (idx > 0) {
                        doc.moveTo(50, rowY - 8).lineTo(545, rowY - 8).lineWidth(0.5).strokeColor('#eeeeee').stroke();
                    }
                    doc.fillColor('#000000').fontSize(13).font('Helvetica-Bold').text(sanitizeText(item.service), 65, rowY);
                    doc.fillColor('#666666').fontSize(10).font('Helvetica').text('Includes 1 free revision', 65, rowY + 16, { width: 220 });
                    doc.fillColor('#000000').fontSize(13).font('Helvetica').text(`Rs.${parseFloat(item.price).toFixed(2)}`, 430, rowY + 4, { width: 100, align: 'right' });
                    rowY += 40;
                });
            } else {
                doc.fillColor('#000000').fontSize(14).font('Helvetica-Bold').text(`${sanitizeText(order.service)} Package`, 65, rowY);
                doc.fillColor('#666666').fontSize(10).font('Helvetica')
                    .text('Premium cinematic editing and post-production. Includes 1 free revision.', 65, rowY + 18, { width: 220 });
                doc.fillColor('#000000').fontSize(14).font('Helvetica').text(formattedDeadline, 295, rowY + 8, { width: 120, align: 'center' });
                doc.fillColor('#000000').fontSize(14).text(`Rs.${safeAmount(parseFloat(order.amount) + (appliedCoupon ? parseFloat(appliedCoupon.discount_amount || 0) : 0))}`, 430, rowY + 8, { width: 100, align: 'right' });
                rowY += 59;
            }

            if (appliedCoupon) {
                doc.moveTo(50, rowY - 8).lineTo(545, rowY - 8).lineWidth(0.5).strokeColor('#eeeeee').stroke();
                doc.fillColor('#22c55e').fontSize(13).font('Helvetica-Bold').text(`Discount (${sanitizeText(appliedCoupon.code)})`, 65, rowY);
                doc.fillColor('#22c55e').fontSize(13).font('Helvetica-Bold').text(`-Rs.${parseFloat(appliedCoupon.discount_amount).toFixed(2)}`, 430, rowY, { width: 100, align: 'right' });
                rowY += 40;
            }

            // Row divider
            doc.moveTo(50, rowY).lineTo(545, rowY).lineWidth(1).strokeColor('#eeeeee').stroke();
            ...
            doc.fillColor('#ff1a1a').fontSize(18).font('Helvetica-Bold')
                .text(`Rs.${safeAmount(order.amount)}`, 390, totalY + 11, { width: 150, align: 'right' }); // UPDATED

            // ── PAYMENT STATUS ──
            const statusY = totalY + 70;
