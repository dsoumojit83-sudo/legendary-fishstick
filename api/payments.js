...
module.exports = async function (req, res) {
    ...
    try {
        ...
        // H-01 FIX: Wrap entire Cashfree fetch loop in a 20s deadline.
        // 6 serial batches × slow Cashfree API (~800ms each) can push past the 30s Vercel limit.
        // If the deadline fires, we return the partial ledger rather than timing out the response.
        let _ledgerTimedOut = false;
        const _ledgerDeadline = new Promise(resolve => setTimeout(() => {
            _ledgerTimedOut = true;
            resolve();
        }, 20000));

        const _ledgerBuild = (async () => {
            for (let i = 0; i < orders.length; i += CHUNK_SIZE) {
                if (_ledgerTimedOut) {
                    console.warn(`[payments.js] Ledger build aborted at chunk ${i} — 20s deadline hit.`);
                    break;
                }
        ...
        await Promise.race([_ledgerBuild, _ledgerDeadline]);
